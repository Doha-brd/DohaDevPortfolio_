...
export default function CertificationsGallery() {
  ...
  // Row 1: Current and Recent studies
  const row1Base = certificationsData.filter((c) =>
    ["python", "sql", "ts-digital-2", "ts-digital-1"].includes(c.id),
  );
  const row1 = row1Base.length > 0 ? [...row1Base, ...row1Base, ...row1Base] : [];

  // Row 2: Earlier studies
  const row2Base = certificationsData.filter((c) =>
    ["tronc-commun-ia", "bac-physique"].includes(c.id),
  );
  const row2 = row2Base.length > 0 ? [...row2Base, ...row2Base, ...row2Base] : [];
